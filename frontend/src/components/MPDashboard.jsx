...
export default function MPDashboard({ selectedState, currentUser, onLogOut }) {
  ...
  useEffect(() => {
    ...
    // Fetch live complaints inline to avoid stale closure issues
    const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || "";
    axios.get(`${API_BASE_URL}/api/get-complaints`, { params: { state: selectedState } })
        ...
        setComplaints([
          {
            ...
            english_translation: "Major leakage in primary water pipeline near Ward 4. Safe drinking water is flowing on the streets, causing water logging and localized traffic jams.",
            image_url: "https://images.unsplash.com/photo-1595267499691-b3b4cf74c86b?auto=format&fit=crop&q=80&w=600",
            severity_index: 8,
            status: "Pending",
            ai_justification: "System flagged this issue with a severity score of 8/10. High structural delay and immediate risk of water contamination in high-density Ward 4."
          },
          {
            request_id: "fall-2",
            category: "Road Damage",
            location_node: "Ward 2",
            state: selectedState,
            submitted_at: new Date(Date.now() - 3600000 * 8).toISOString(),
            english_translation: "Dangerous pothole array at the main highway crossing leading to frequent motorcycle slip-ups and transit congestion.",
            image_url: "https://images.unsplash.com/photo-1515162305285-0293e4767cc2?auto=format&fit=crop&q=80&w=600",
            severity_index: 7,
            status: "In Progress",
            ai_justification: "Traffic flow impedance coupled with high vulnerability of two-wheeler transit corridors near highway crossings."
          },
          {
            request_id: "fall-3",
            category: "Sanitation",
            location_node: "Ward 1",
            state: selectedState,
            submitted_at: new Date(Date.now() - 3600000 * 24).toISOString(),
            english_translation: "Domestic waste collector vehicles have not arrived for a week. Piles of garbage are causing severe sanitary hazards near local schools.",
            image_url: null,
            severity_index: 5,
            status: "Resolved",
            ai_justification: "Risk indices lowered following primary waste collection dispatch to clear residential dumpster heaps."
          }
        ]);
      });
  }, [selectedState]); // Only depend on selectedState — stateData is derived from it
