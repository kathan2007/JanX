...
export default function ResidentPortal({ selectedState, onTriggerAuthModal, currentUser }) {
  ...
  const handleFormSubmit = async (e, injectedUser = null) => {
    ...
    try {
      ...
      // 🎯 Is pure try-catch block ko apne handle submit function ke andar paste kar do
      try {
        // 1. Api URL logic with absolute fallback
        const API_BASE_URL = import.meta.env.VITE_API_URL || "https://janx.onrender.com";
        let categoryResponse = "Infrastructure Gap";

        // 2. Axios POST call directly to live Render server
        ...
        // 5. Fallback local categorizer logic based on input keywords
        ...
        // Fallback ke case mein error alert handle karna chaho toh:
